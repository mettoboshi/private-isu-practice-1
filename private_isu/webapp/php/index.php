...
$app->get('/', function (Request $request, Response $response) {
    ...
    $db = $this->get('db');
    $ps = $db->prepare('SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` left join `users` on `posts`.`user_id` = `users`.`id` where `users`.`del_flg` = 0 ORDER BY `posts`.`created_at` DESC LIMIT 20');
    $ps->execute();
...
$app->get('/posts', function (Request $request, Response $response) {
    ...
    $db = $this->get('db');
    $ps = $db->prepare('SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` left join `users` ON `posts`.`user_id` = `users`.`id` WHERE `users`.`del_flg` = 0 AND `posts`.`created_at` <= ? ORDER BY `posts`.`created_at` DESC LIMIT 20');
...
$app->get('/@{account_name}', function (Request $request, Response $response, $args) {
    ...
    $ps = $db->prepare('SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` left join `users` on `posts`.`user_id` = `users`.`id` where `users`.`del_flg` = 0 and `posts`.`user_id` = ? ORDER BY `posts`.`created_at` DESC LIMIT 20');
    $ps->execute([$user['id']]);
    $results = $ps->fetchAll(PDO::FETCH_ASSOC);
    $posts = $this->get('helper')->make_posts($results);
