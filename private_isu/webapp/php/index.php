...
$app->get('/', function (Request $request, Response $response) {
    ...
    $db = $this->get('db');
    $ps = $db->prepare('SELECT `posts`.`id`, `posts`.`user_id`, `posts`.`body`, `posts`.`mime`, `posts`.`created_at` FROM `posts` left join `users` on `posts`.`user_id` = `users`.`id` where `users`.`del_flg` = 0 ORDER BY `created_at` DESC LIMIT 20');
    $ps->execute();
